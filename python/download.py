...
def main():
    df = pd.DataFrame()
    tickers = sys.argv[1:]
    apikey = open(f"./apikey", "r").readline()
    for ticker in tickers:
        ...
        hist = pd.read_csv(f"./data/{ticker}.csv").loc[::-1][["date", "adjClose"]]
    ...
    df.to_csv(f"./data/merge.csv", index=False)
